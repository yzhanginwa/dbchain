...
type SessionStore struct {
	...
	value            map[string]string //session 里面存储的值
}

//设置
func (st *SessionStore) Set(key, value string) error {
	st.value[key] = value
...
//获取session
func (st *SessionStore) Get(key string) string {
	pder.SessionUpdate(st.sid)
...
func (frommemory *FromMemory) SessionRead(sid string) (Session, error) {
	frommemory.lock.RLock()
	if element, ok := frommemory.sessions[sid]; ok {
		frommemory.lock.RUnlock()
