...
func lnumberValue(expr ast.Expr) (lua.LNumber, bool) {
	if ex, ok := expr.(*ast.NumberExpr); ok {
		lv, err := parseNumber(ex.Value)
		if err != nil {
			lv = lua.LNumber(math.NaN())
		}
		return lv, true
	} else if ex, ok := expr.(*constLValueExpr); ok {
		return ex.Value.(lua.LNumber), true
	}
	return 0, false
}
...
func (cd *codeStore) AddLoadNil(a, b, line int) {
	last := cd.Last()
	if opGetOpCode(last) == lua.OP_LOADNIL && (opGetArgA(last)+opGetArgB(last)) == a {
		cd.SetB(cd.LastPC(), b)
	} else {
		cd.AddABC(lua.OP_LOADNIL, a, b, 0, line)
	}
}
...
func newFunctionProto(name string) *lua.FunctionProto {
...
func (fc *funcContext) ConstIndex(value lua.LValue) int {
	ctype := value.Type()
...
func (fc *funcContext) RegisterLocalVar(name string) int {
	...
	fc.Proto.DbgLocals = append(fc.Proto.DbgLocals, &lua.DbgLocalInfo{Name: name, StartPc: fc.Code.LastPC() + 1})
...
func (fc *funcContext) CloseUpvalues() int {
	...
	if fc.Block.RefUpvalue {
		...
		fc.Code.AddABC(lua.OP_CLOSE, n, 0, 0, fc.Block.LastLine)
	}
	return n
}
...
func compileChunk(context *funcContext, chunk []ast.Stmt) bool { // {{{
	for _, stmt := range chunk {
		hasLoop := compileStmt(context, stmt)
		if hasLoop {
			return hasLoop
		}
	}
...
func compileStmt(context *funcContext, stmt ast.Stmt) bool { // {{{
	switch st := stmt.(type) {
	case *ast.AssignStmt:
...
func compileAssignStmtLeft(context *funcContext, stmt *ast.AssignStmt) (int, []*assigncontext) { // {{{
	...
	for i, lhs := range stmt.Lhs {
		...
		case *ast.IdentExpr:
			...
			switch identtype {
			case ecGlobal:
				context.ConstIndex(lua.LString(st.Value))
			case ecUpvalue:
...
func compileAssignStmt(context *funcContext, stmt *ast.AssignStmt) { // {{{
	...
	for i := lennames - 1; i >= 0; i-- {
		...
		case ecLocal:
			if acs[i].needmove {
				code.AddABC(lua.OP_MOVE, context.FindLocalVar(ex.(*ast.IdentExpr).Value), reg, 0, sline(ex))
				reg -= 1
			}
		case ecGlobal:
			code.AddABx(lua.OP_SETGLOBAL, reg, context.ConstIndex(lua.LString(ex.(*ast.IdentExpr).Value)), sline(ex))
			reg -= 1
		case ecUpvalue:
			code.AddABC(lua.OP_SETUPVAL, reg, context.Upvalues.RegisterUnique(ex.(*ast.IdentExpr).Value), 0, sline(ex))
			reg -= 1
		case ecTable:
			opcode := lua.OP_SETTABLE
			if acs[i].keyks {
				opcode = lua.OP_SETTABLEKS
			}
...
func compileReturnStmt(context *funcContext, stmt *ast.ReturnStmt) { // {{{
	...
	if lenexprs == 1 {
		...
		case *ast.IdentExpr:
			if idx := context.FindLocalVar(ex.Value); idx > -1 {
				code.AddABC(lua.OP_RETURN, idx, 2, 0, sline(stmt))
				return
			}
		case *ast.FuncCallExpr:
			reg += compileExpr(context, reg, ex, ecnone(-2))
			code.SetOpCode(code.LastPC(), lua.OP_TAILCALL)
			code.AddABC(lua.OP_RETURN, a, 0, 0, sline(stmt))
			return
		}
	}
	...
	context.Code.AddABC(lua.OP_RETURN, a, count, 0, sline(stmt))
} // }}}
...
func compileIfStmt(context *funcContext, stmt *ast.IfStmt) { // {{{
	...
	if len(stmt.Else) > 0 {
		context.Code.AddASbx(lua.OP_JMP, 0, endlabel, sline(stmt))
	}
...
func compileBranchCondition(context *funcContext, reg int, expr ast.Expr, thenlabel, elselabel int, hasnextcond bool) { // {{{
	...
	case *ast.FalseExpr, *ast.NilExpr:
		if !hasnextcond {
			code.AddASbx(lua.OP_JMP, 0, elselabel, sline(expr))
			return
		}
	...
	code.AddABC(lua.OP_TEST, a, 0, 0^flip, sline(expr))
	code.AddASbx(lua.OP_JMP, 0, jumplabel, sline(expr))
} // }}}
...
func compileBreakStmt(context *funcContext, stmt *ast.BreakStmt) { // {{{
	for block := context.Block; block != nil; block = block.Parent {
		if label := block.BreakLabel; label != labelNoJump {
			if block.RefUpvalue {
				context.Code.AddABC(lua.OP_CLOSE, block.Parent.LocalVars.LastIndex(), 0, 0, sline(stmt))
			}
			context.Code.AddASbx(lua.OP_JMP, 0, label, sline(stmt))
			return
		}
	}
...
func compileFuncDefStmt(context *funcContext, stmt *ast.FuncDefStmt) { // {{{
	if stmt.Name.Func == nil {
		...
		kreg = loadRk(context, &reg, stmt.Func, lua.LString(stmt.Name.Method))
		compileExpr(context, reg, stmt.Func, ecfuncdef)
		context.Code.AddABC(lua.OP_SETTABLE, treg, kreg, reg, sline(stmt.Name.Receiver))
	} else {
...
func compileExpr(context *funcContext, reg int, expr ast.Expr, ec *expcontext) int { // {{{
	...
	switch ex := expr.(type) {
	case *ast.StringExpr:
		code.AddABx(lua.OP_LOADK, sreg, context.ConstIndex(lua.LString(ex.Value)), sline(ex))
		return sused
	case *ast.NumberExpr:
		num, err := parseNumber(ex.Value)
		if err != nil {
			num = lua.LNumber(math.NaN())
		}
		code.AddABx(lua.OP_LOADK, sreg, context.ConstIndex(num), sline(ex))
		return sused
	case *constLValueExpr:
		code.AddABx(lua.OP_LOADK, sreg, context.ConstIndex(ex.Value), sline(ex))
		return sused
	case *ast.NilExpr:
		code.AddLoadNil(sreg, sreg, sline(ex))
		return sused
	case *ast.FalseExpr:
		code.AddABC(lua.OP_LOADBOOL, sreg, 0, 0, sline(ex))
		return sused
	case *ast.TrueExpr:
		code.AddABC(lua.OP_LOADBOOL, sreg, 1, 0, sline(ex))
		return sused
	case *ast.IdentExpr:
		switch getIdentRefType(context, context, ex) {
		case ecGlobal:
			code.AddABx(lua.OP_GETGLOBAL, sreg, context.ConstIndex(lua.LString(ex.Value)), sline(ex))
		case ecUpvalue:
			code.AddABC(lua.OP_GETUPVAL, sreg, context.Upvalues.RegisterUnique(ex.Value), 0, sline(ex))
		case ecLocal:
			b := context.FindLocalVar(ex.Value)
			code.AddABC(lua.OP_MOVE, sreg, b, 0, sline(ex))
		}
		return sused
	case *ast.Comma3Expr:
		...
		context.Proto.IsVarArg &= ^lua.VarArgNeedsArg
		code.AddABC(lua.OP_VARARG, sreg, 2+ec.varargopt, 0, sline(ex))
	...
	case *ast.AttrGetExpr:
		...
		opcode := lua.OP_GETTABLE
		if _, ok := ex.Key.(*ast.StringExpr); ok {
			opcode = lua.OP_GETTABLEKS
		}
	...
	case *ast.FunctionExpr:
		...
		for _, upvalue := range childcontext.Upvalues.List() {
			...
			if localidx > -1 {
				...
				code.AddABC(lua.OP_GETUPVAL, 0, upvalueidx, 0, sline(ex))
			}
		}
		return sused
	default:
...
func constFold(exp ast.Expr) ast.Expr {
	// {{{
	luaModulo := func(lhs, rhs lua.LNumber) lua.LNumber {
		flhs := float64(lhs)
		frhs := float64(rhs)
	...
	case *ast.ArithmeticOpExpr:
		...
		if lisconst && risconst {
			...
			case "^":
				return &constLValueExpr{Value: lua.LNumber(math.Pow(float64(lvalue), float64(rvalue)))}
			default:
	...
	case *ast.UnaryMinusOpExpr:
		...
		if value, ok := lnumberValue(expr.Expr); ok {
			return &constLValueExpr{Value: lua.LNumber(-value)}
		}
		return expr
	default:
...
func compileFunctionExpr(context *funcContext, funcexpr *ast.FunctionExpr, ec *expcontext) { // {{{
	...
	if funcexpr.ParList.HasVargs {
		if lua.CompatVarArg {
			context.Proto.IsVarArg = lua.VarArgHasArg | lua.VarArgNeedsArg
			if context.Parent != nil {
				context.RegisterLocalVar("arg")
			}
		}
		context.Proto.IsVarArg |= lua.VarArgIsVarArg
	}

	compileChunk(context, funcexpr.Stmts)

	context.Code.AddABC(lua.OP_RETURN, 0, 1, 0, eline(funcexpr))
	context.EndScope()
...
func compileTableExpr(context *funcContext, reg int, ex *ast.TableExpr, ec *expcontext) { // {{{
	...
	*/
	tablereg := reg
	reg++
	code.AddABC(lua.OP_NEWTABLE, tablereg, 0, 0, sline(ex))
	tablepc := code.LastPC()
	regbase := reg
	...
	for i, field := range ex.Fields {
		...
		flush := arraycount % lua.FieldsPerFlush
		if (arraycount != 0 && (flush == 0 || islast)) || lastvararg {
			reg = regbase
			num := flush
			if num == 0 {
				num = lua.FieldsPerFlush
			}
			c := (arraycount-1)/lua.FieldsPerFlush + 1
			b := num
			if islast && isVarArgReturnExpr(field.Value) {
				b = 0
			}
			...
			code.AddABC(lua.OP_SETLIST, tablereg, b, c, sline(line))
			if c == 0 {
	...
	if shouldmove(ec, tablereg) {
		code.AddABC(lua.OP_MOVE, ec.reg, tablereg, 0, sline(ex))
	}
} // }}}
...
func compileStringConcatOpExpr(context *funcContext, reg int, expr *ast.StringConcatOpExpr, ec *expcontext) { // {{{
	...
	for pc := code.LastPC(); pc != 0 && opGetOpCode(code.At(pc)) == lua.OP_CONCAT; pc-- {
		code.Pop()
	}
	code.AddABC(lua.OP_CONCAT, a, basereg, basereg+crange, sline(expr))
} // }}}
...
func compileUnaryOpExpr(context *funcContext, reg int, expr ast.Expr, ec *expcontext) { // {{{
	...
	case *ast.UnaryNotOpExpr:
		switch ex.Expr.(type) {
		case *ast.TrueExpr:
			code.AddABC(lua.OP_LOADBOOL, savereg(ec, reg), 0, 0, sline(expr))
			return
		case *ast.FalseExpr, *ast.NilExpr:
			code.AddABC(lua.OP_LOADBOOL, savereg(ec, reg), 1, 0, sline(expr))
			return
		default:
			opcode = lua.OP_NOT
			operandexpr = ex.Expr
...
func compileRelationalOpExprAux(context *funcContext, reg int, expr *ast.RelationalOpExpr, flip int, label int) { // {{{
	...
	switch expr.Operator {
	case "<":
		code.AddABC(lua.OP_LT, 0^flip, b, c, sline(expr))
	case ">":
		code.AddABC(lua.OP_LT, 0^flip, c, b, sline(expr))
	case "<=":
		code.AddABC(lua.OP_LE, 0^flip, b, c, sline(expr))
	case ">=":
		code.AddABC(lua.OP_LE, 0^flip, c, b, sline(expr))
	case "==":
		code.AddABC(lua.OP_EQ, 0^flip, b, c, sline(expr))
	case "~=":
		code.AddABC(lua.OP_EQ, 1^flip, b, c, sline(expr))
	}
	code.AddASbx(lua.OP_JMP, 0, label, sline(expr))
} // }}}
...
func compileRelationalOpExpr(context *funcContext, reg int, expr *ast.RelationalOpExpr, ec *expcontext) { // {{{
	...
	code.AddABC(lua.OP_LOADBOOL, a, 0, 1, sline(expr))
	context.SetLabelPc(jumplabel, code.LastPC())
	code.AddABC(lua.OP_LOADBOOL, a, 1, 0, sline(expr))
} // }}}
...
func compileLogicalOpExpr(context *funcContext, reg int, expr *ast.LogicalOpExpr, ec *expcontext) { // {{{
	...
	lastinst := code.Last()
	if opGetOpCode(lastinst) == lua.OP_JMP && opGetArgSbx(lastinst) == endlabel {
		code.Pop()
	}
...
func compileLogicalOpExprAux(context *funcContext, reg int, expr ast.Expr, ec *expcontext, thenlabel, elselabel int, hasnextcond bool, lb *lblabels) { // {{{
	...
	case *ast.FalseExpr:
		if elselabel == lb.e {
			code.AddASbx(lua.OP_JMP, 0, lb.f, sline(expr))
			lb.b = true
		} else {
			code.AddASbx(lua.OP_JMP, 0, elselabel, sline(expr))
		}
	...
	case *ast.NilExpr:
		if elselabel == lb.e {
			...
			code.AddASbx(lua.OP_JMP, 0, lb.e, sline(expr))
		} else {
			code.AddASbx(lua.OP_JMP, 0, elselabel, sline(expr))
		}
		return
	case *ast.TrueExpr:
		if thenlabel == lb.e {
			code.AddASbx(lua.OP_JMP, 0, lb.t, sline(expr))
			lb.b = true
		} else {
			code.AddASbx(lua.OP_JMP, 0, thenlabel, sline(expr))
		}
		return
	...
	if !hasnextcond && thenlabel == elselabel {
		...
		if opGetOpCode(last) == lua.OP_MOVE && opGetArgA(last) == a {
			context.Code.SetA(context.Code.LastPC(), sreg)
		} else {
			context.Code.AddABC(lua.OP_MOVE, sreg, a, 0, sline(expr))
		}
	} else {
		reg += compileExpr(context, reg, expr, ecnone(0))
		if sreg == a {
			...
			code.AddABC(lua.OP_TESTSET, sreg, a, 0^flip, sline(expr))
		}
	}
	code.AddASbx(lua.OP_JMP, 0, jumplabel, sline(expr))
} // }}}
...
func compileFuncCallExpr(context *funcContext, reg int, expr *ast.FuncCallExpr, ec *expcontext) int { // {{{
	...
	if expr.Func != nil { // hoge.func()
		...
		c := loadRk(context, &reg, expr, lua.LString(expr.Method))
		context.Code.AddABC(lua.OP_SELF, funcreg, b, c, sline(expr))
	...
	context.Code.AddABC(lua.OP_CALL, funcreg, b, ec.varargopt+2, sline(expr))
	context.Proto.DbgCalls = append(context.Proto.DbgCalls, lua.DbgCall{Pc: context.Code.LastPC(), Name: name})

	if ec.varargopt == 0 && shouldmove(ec, funcreg) {
		context.Code.AddABC(lua.OP_MOVE, ec.reg, funcreg, 0, sline(expr))
		return 1
	}
...
func loadRk(context *funcContext, reg *int, expr ast.Expr, cnst lua.LValue) int { // {{{
	...
	if cindex <= opMaxIndexRk {
		...
		ret := *reg
		*reg++
		context.Code.AddABx(lua.OP_LOADK, ret, cindex, sline(expr))
		return ret
	}
} // }}}
...
func patchCode(context *funcContext) { // {{{
	...
	for pc := 0; pc < len(code); pc++ {
		...
		case lua.OP_CLOSURE:
			...
			moven = 0
			continue
		case lua.OP_SETGLOBAL, lua.OP_SETUPVAL, lua.OP_EQ, lua.OP_LT, lua.OP_LE, lua.OP_TEST,
			lua.OP_TAILCALL, lua.OP_RETURN, lua.OP_FORPREP, lua.OP_FORLOOP, lua.OP_TFORLOOP,
			lua.OP_SETLIST, lua.OP_CLOSE:
			/* nothing to do */
		case lua.OP_CALL:
		...
		case lua.OP_JMP: // jump to jump optimization
			distance := 0
			count := 0 // avoiding infinite loops
			for jmp := inst; opGetOpCode(jmp) == lua.OP_JMP && count < 5; jmp = context.Code.At(pc + distance + 1) {
			...
			if distance == 0 {
				context.Code.SetOpCode(pc, lua.OP_NOP)
			} else {
		...
		if curop == lua.OP_MOVE {
			moven++
		} else {
			if moven > 1 {
				context.Code.SetOpCode(pc-moven, lua.OP_MOVEN)
